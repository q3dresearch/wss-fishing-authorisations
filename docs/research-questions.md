...
| F5 | Was a departure an enforcement action or an ordinary exit? | **answered, 2026-09-11 — and it is the sharpest thing in this repository** | The source exists, it is a larder, and **it is published by the same secretariat in the same file format**. `Data/IUU/IUU.xlsx` has TWO sheets: `IUUs`, 185 current listings, and **`HstIUUs`, 247 historical ones with an extra column — `DateToHist`, filled on 100% of them.** So ICCAT records the date every single vessel came OFF its IUU list, back nineteen years, while recording the date not one vessel came off its authorisation record. Median time on the IUU list is **1,874 days (5.1 years)**, longest 6,922 (19.0 years), with 130 delistings in 2021 alone. Fetch once as a join control; do not add a registry entry |
